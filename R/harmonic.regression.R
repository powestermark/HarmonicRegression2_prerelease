...
harmonic_regression_matrix_nuisance <- function(inputts, inputtime, Tau,
  ...
  # New solution, avoiding parent.env<- which is dangerous:
  # copy of local function environment, sets the formula environment as its
  # parent
  new_env <- list2env(as.list(environment(), all.names = TRUE),
                      parent = environment(nuisance_f))
  # sets this as the new formula environment
  environment(nuisance_f) <- new_env
...
normalize_one_ts_r_pol <- function(inputts, inputtime, norm.pol.degree,
  ...
  tau_delta <- ifelse(n.non.na/3 < 6, 0.95, 0.8)
  ...
  if (!isa(rest.fit, "try-error")) {
    ...
    unrest.fit <- try(
      Rfit::rfit(unrest_f, na.action = na.exclude, scores = robust_scores,
                 yhat0 = rest_fit.vals, delta = tau_delta),
      silent = TRUE
    )
  }
  # else {
...
#'  \code{ci} \tab Data frame of one-sided approximate 95% (\eqn{2\sigma})
